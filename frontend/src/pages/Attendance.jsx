...
export default function Attendance() {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Chấm công vào thất bại');
  ...
  const handleCheckOut = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Chấm công ra thất bại');
    } finally { setCicoLoading(false); }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="page-subtitle">Theo dõi giờ vào và giờ ra hàng ngày</p>
        </div>
        {isEmployee() && (
          <button onClick={() => setCicoModal(true)} className="btn-primary">
            <CalendarDays size={16} />
            {todayStatus?.checked_in ? (todayStatus?.checked_out ? 'Xem hôm nay' : 'Chấm ra') : 'Chấm vào'}
          </button>
        )}
      </div>
        ...
        <div className="card p-5 border border-primary-100 dark:border-primary-900/30">
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="flex-1">
              <p className="font-semibold text-slate-800 dark:text-white">Trạng thái hôm nay</p>
              <p className="text-xs text-slate-400 mt-0.5">{new Date().toLocaleDateString('vi-VN', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
            </div>
      ...
      <div className="card p-4">
        <div className="flex flex-wrap gap-3">
          <div className="flex items-center gap-2 flex-shrink-0 text-slate-500 dark:text-slate-400">
            <Filter size={15} /> <span className="text-sm font-medium">Lọc</span>
          </div>
          <input
            type="date"
          ...
          <button onClick={() => setFilters({ date: '', employee_id: '', department: '', status: '' })}
            className="btn-ghost text-xs">Xóa lọc</button>
        </div>
      </div>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="6" className="text-center py-12">
                  ...
                  <p className="text-slate-400 text-sm">Không tìm thấy dữ liệu</p>
                </td></tr>
              ) : records.map(r => (
                <tr key={r.id}>
                  <td>
      ...
      >
        <div className="text-center space-y-6 py-2">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">{now.toLocaleDateString('vi-VN', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
          </div>
          ...
          {todayStatus && (
            <div className="bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 text-sm space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-slate-700 dark:text-white">{todayStatus.record?.check_in || 'Chưa chấm'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Giờ ra:</span>
                <span className="font-semibold text-slate-700 dark:text-white">{todayStatus.record?.check_out || 'Chưa chấm'}</span>
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col gap-3">
            {!todayStatus?.checked_in && (
              <button onClick={handleCheckIn} disabled={cicoLoading} className="btn-primary py-3 text-base justify-center">
                {cicoLoading ? <span className="spinner border-white/30 border-t-white" /> : <><LogIn size={18} /> Chấm vào ngay</>}
              </button>
            )}
            {todayStatus?.checked_in && !todayStatus?.checked_out && (
              <button onClick={handleCheckOut} disabled={cicoLoading} className="btn btn-danger bg-red-500 text-white py-3 text-base justify-center">
                {cicoLoading ? <span className="spinner border-white/30 border-t-white" /> : <><LogOut size={18} /> Chấm ra ngay</>}
              </button>
            )}
            {todayStatus?.checked_in && todayStatus?.checked_out && (
              <div className="badge-success py-2.5 text-sm justify-center">✓ Đã hoàn thành chấm công hôm nay</div>
            )}
          </div>
        </div>
      </Modal>
